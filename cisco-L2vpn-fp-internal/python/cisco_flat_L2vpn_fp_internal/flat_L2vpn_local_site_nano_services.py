...
class FlatL2vpnValidator(object):
    ...
    def cb_validate(self, tctx, kp, newval):
        return TsdnUtils.validate_service(self, Utils.local_site_validation_callpoint, tctx, kp)


class FlatL2vpnCallBack(FlatL2vpnServices):
    @ncs.application.Service.pre_modification
    @instrumentation.instrument_service(logging.INFO, Utils.local_site_servicepoint)
    def cb_pre_modification(self, tctx, op, kp, root, proplist):
        return self.l2vpn_pre_modification(constants.LOCAL_SITE, tctx, op, kp, root, proplist)
...
class FlatL2vpnSelfCallBack(ncs.application.NanoService):
    """
    NanoService callback handler for flat-L2vpn service local-site
    """

    @ncs.application.NanoService.create
    @instrumentation.instrument_nano(logging.INFO, Utils.local_site_servicepoint)
    def cb_nano_create(self, tctx, root, service, plan,
        ...
        try:
            new_opaque = None
            if state == "cisco-flat-L2vpn-fp-local-site-nano-services:config-apply":
    ...
    def _create_config_apply(self, tctx, root, service, plan,
        ...
        try:
            ...
            raise DeviceConfigException(
                ...
                "Configuration Error", "Could not apply service config on local site"
            ).add_state(
                "Local Site", site.pe
            ).add_state(
                "Service", service.name
